...
export interface Style2DProperties extends IDOCTextStyleDeclaration, IDOCTransformStyleDeclaration {
...
export interface Style2D extends Style2DProperties {
  //
}
...
export class Style2D extends Resource {
  ...
  constructor(properties?: Partial<Style2DProperties>) {
    super()
  ...
  getComputedFilter(): Style2DFilter {
...
for (const key in transformStyle) {
  defineProperty(Style2D, key, { default: (transformStyle as any)[key] })
}

const textStyle = getDefaultTextStyle()
for (const key in textStyle) {
  defineProperty(Style2D, key, { default: (textStyle as any)[key] })
}
